<template>
  ...
  <div class="vh-100 d-flex justify-content-center align-items-center">
    <div class="card p-4 shadow-lg" style="width: 22rem;">
      <h3 class="text-center mb-1 text-primary">Join GradesHub</h3>
      <p class="text-center mb-3 text-muted">Create your account</p>
      <form @submit.prevent="handleRegister">
        <div class="mb-3">
          <label class="form-label">First Name</label>
          <input type="text" class="form-control" placeholder="Enter first name" v-model="firstName" @input="clearFirstNameError" />
          <span class="text-danger" v-if="firstNameError">{{ firstNameError }}</span>
        </div>
        <div class="mb-3">
          <label class="form-label">Last Name</label>
          <input type="text" class="form-control" placeholder="Enter last name" v-model="lastName" @input="clearLastNameError" />
          <span class="text-danger" v-if="lastNameError">{{ lastNameError }}</span>
        </div>
        <div class="mb-3">
          <label class="form-label">Email Address</label>
          <input type="email" class="form-control" placeholder="Enter email" v-model="email" @input="clearEmailError" />
